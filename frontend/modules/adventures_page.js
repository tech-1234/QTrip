...
async function fetchAdventures(city) {
  ...
  return fetch(API_URL + `/adventures?city=${city}`)
  .then(res => res.json())
  .catch(err => {
    return null;
  })
  ...
  adventures.forEach(adventure => {
    ...
    <a href="detail/?adventure=${adventure.id}" id=${adventure.id}>
      <div class="activity-card">
        ...
        <div class="w-100 d-flex justify-content-around px-2 py-1">
...
function filterByDuration(list, low, high) {
  ...
  list.forEach((listItem) => {
    if(listItem.duration >= low && listItem.duration <= high) {
...
function generateFilterPillsAndUpdateDOM(filters) {
  ...
  category.forEach((list) => {
    ...
    let innerHTML = `<div class="category-filter">${list}</div>`
    container.innerHTML = innerHTML;
